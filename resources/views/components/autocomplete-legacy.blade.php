...
<x-autocomplete :auto-select="$autoSelect" :wire:model.live="$selectedProperty->value">
    <x-autocomplete-input :wire:model.live="$inputProperty->value" class="bg-white" x-bind:disabled="id" />
            ...
            @if ($hasResults($resultsValue) || $allowNew)
                ...
                @if ($resultsValue)
                    @foreach ($resultsValue as $key => $result)
                        ...
                        <x-autocomplete-item :key="$result[$getOption('id')] ?? $key" :value="$result[$getOption('text')] ?? $result" wire:key="{{ $name }}-result-{{ $key }}">
